...
void EmuNVNet_DMAPacketFromGuest()
{
	...
	for (int i = 0; i < s->tx_ring_size; i++) {
		...
		DbgPrintf("Looking at ring desc %d (%llx): ", s->tx_ring_index, tx_ring_addr);
		DbgPrintf("Buffer: 0x%x, ", desc.packet_buffer);
		DbgPrintf("Length: 0x%x, ", desc.length);
		...
		if (is_last_packet) {
			DbgPrintf("  -- Last packet\n");
			break;
		}
	}

	/* Trigger interrupt */
	DbgPrintf("Triggering interrupt\n");
...
bool EmuNVNet_DMAPacketToGuest(void* packet, size_t size)
{
	...
	for (int i = 0; i < s->rx_ring_size; i++) {
		...
		DbgPrintf("Looking at ring descriptor %d (0x%llx): ", s->rx_ring_index, rx_ring_addr);
		DbgPrintf("Buffer: 0x%x, ", desc.packet_buffer);
		DbgPrintf("Length: 0x%x, ", desc.length);
		DbgPrintf("Flags: 0x%x\n", desc.flags);
		...
		DbgPrintf("Transferring packet, size 0x%zx, to memory at 0x%x\n", size, desc.packet_buffer);
		...
		DbgPrintf("Updated ring descriptor: ");
		DbgPrintf("Length: 0x%x, ", desc.length);
		DbgPrintf("Flags: 0x%x\n", desc.flags);
	...
	EmuWarning("Could not find free buffer!\n");
	return false;
}
...
bool NVNetDevice::PCAPInit()
{
	...
	if (pcap_setnonblock((pcap_t*)m_AdapterHandle, 1, errorBuffer) == -1) {
		EmuWarning("PCAP: Failed to set non-blocking mode");
	}
...
size_t NVNetDevice::PCAPReceive(void* packet, size_t max_length)
{
	...
	if (int res = pcap_next_ex((pcap_t*)m_AdapterHandle, &header, &pkt_data) > 0) {
		// Only forward packets that are multicast or specificlaly for Cxbx-R's MAC
